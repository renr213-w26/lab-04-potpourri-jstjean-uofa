...
a.  Add each of strings below to the code chunk provided in your document, render the document, and determine if the string is a proper code chunk label. If not, explain why and describe how you could fix it so the document renders.
    ...
    **Comment: It cannot go over two lines so I had to make it all into one line for it to render**
    ...
    **Comment: It does not allow for spaces to be in labels so I had to put dashes in between the words to allow it to render**
    ...
    **Comment: This works with no changes**
...
b.  Which of the chunk label options above is the best option? Explain your reasoning in 1-2 sentences.
    ...
    **Comment: The label with dashes is the best option, it looks the cleanest and allows the document to render properly.**
...
**Echo: decides if the code if shown in the rendered document (shown when true, absent when false)**

**Warning: decides if the warning messages are show in the output (shown when true, absent when false)**
...
The following chart was [shared](https://twitter.com/GraphCrimes/status/1566264004288331776) by \@GraphCrimes on X/Twitter on September 3, 2022. What is misleading about this graph?

(I had to remove the file link to the graph because it wasn't working)

a.  Suppose you wanted to recreate this plot, with improvements to avoid its misleading pitfalls from part (a). You would obviously need the data from the survey in order to be able to do that. How many observations would this data have? How many variables (at least) should it have, and what should those variables be?\
    \
    **Comment: The data would have 1858 observations, it should have at least 2 variables, the service (rail, water, energy, royal mail) and the response (public, private, don't know).**\
b.  Load the data for this survey from `data/survation.csv`. Confirm that the data match the percentages from the visualization. That is, calculate the percentages of public sector, private sector, don't know for each of the services and check that they match the percentages from the plot.\
    \
    **Comment: I cannot do this question because there is no survation.csv in the project files**
...
**Comment: I cannot do this question without the survation data.**
...
**Comment:** The statistics are very similar across all the datasets.
...
**Comment:** Each of the datasets has a completely different visual pattern made with its points. Compared to the results from the last question (where all of the statistics are very similar) it shows that statistics can possibly be misleading and visualization is important when you want to understand the data fully.
